...
class RGBot extends EventEmitter {
     ...
     * @param {boolean} [isPython] Whether this bot is loaded from python runtime.  Python runtime does not support js timer functions.
     */
    constructor(bot, isPython=false) {
        super();
    ...
    async waitForMilliseconds(milliseconds) {
        ...
        if (!this.#isPython) {
            ...
            // python can't use timers, wait ticks instead
            const toWait = milliseconds/50
            // if not an even 50, tack on 1 tick
            const extra = milliseconds%50 > 0 ? 1 : 0
            await this.wait( toWait+extra)
        }
    }
    ...
    async equipBestHarvestTool(block) {
        ...
        if (bestHarvestTool) {
            try {
                ...
                return new Promise( resolve => this.#bot.heldItem);
            } catch (err) {
