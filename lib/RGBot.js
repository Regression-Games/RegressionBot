...
const { pathfinder, Pathfinder, Movements } = require('mineflayer-pathfinder');
...
class RGBot extends EventEmitter {
    /**
     * @typedef { import('mineflayer-pathfinder').Bot } Bot
     * @typedef { import('mineflayer-pathfinder').Pathfinder } Pathfinder
     ...
     * @param {boolean} [options.isPython=false] Whether this bot is loaded from python runtime.  Python runtime does not support js timer functions.
     */
    constructor(bot, options = {}) {
        super();

        this.#isPython = options.isPython || false;
     ...
     * @return {Pathfinder} The mineflayer-pathfinder instance used by the RGBot
     * @see {@link https://github.com/PrismarineJS/mineflayer-pathfinder}
     *
     * @example <caption>Accessing pathfinder API</caption>
     ...
     */
    on(event, func) {
        if (this.#RG_EVENTS.includes(event) || event.startsWith("rg_")) {
            // if this is one of the RGBot/RGMatchInfo events, or from a user plugin, handle it
            super.on(event, func);
        } else {
            // forward to mineflayer
    ...
    off(event, func) {
        if (this.#RG_EVENTS.includes(event) || event.startsWith("rg_")) {
            // if this is one of the RGBot/RGMatchInfo events, or from a user plugin, handle it
            super.off(event, func);
        } else {
            // forward to mineflayer
            this.#bot.off(event, func);
        }
        return this;
    }
     ...
     * @return {string|null} Name of the team that this bot is on
     */
    getMyTeam() {
        return this.teamForPlayer(this.username())
    }

    /**
     * Returns a list of all players on the same team as this bot.
     * @param {boolean} includeSelf if true, includes this bot in the list
     * @return {string[]} A list of all player usernames on the same team as this bot.
     */
     ...
     * @return {string[]} A list of all player usernames on the enemy team.
     */
     ...
     * @return {Function | number} The function or timerId scheduled. Can be passed to rgbot.clearTimeout to end this timer
     */
    setTimeout(action, time) {
        if( !this.#isPython) {
            ...
            const tickListener = async () => {
                --ticks
                if (ticks <= 0) {
                    this.removeListener('physicsTick', tickListener)
                    await action()
                }
            }
            this.on('physicsTick', tickListener)
            return tickListener
        }
    }
     ...
     * @param {Function} action Action to perform
     * @param {number} time Time in milliseconds
     * @return {Function | number} The function or timerId scheduled as an interval.  Can be passed to rgbot.clearInterval to end this interval
     */
    setInterval(action, time)  {
        if( !this.#isPython) {
            return setInterval(action, time)
        } else {
            const ticks = time/50 + 1
            let ticker = 0
            const tickListener = async () => {
                ++ticker
                if (ticker % ticks == 0) {
     ...
     * @param {Function | number} timerRef The function or timerId to clear
     */
    clearTimeout(timerRef) {
        if (!this.#isPython) {
            clearTimeout(timerRef)
        } else {
            this.off('physicsTick', timerRef)
        }
    }
     ...
     * @param {Function | number} intervalRef The function or intervalId to clear
     */
    clearInterval(intervalRef) {
        if (!this.#isPython) {
            clearInterval(intervalRef)
        } else {
            this.off('physicsTick', intervalRef)
        }
    }
    ...
    async handlePath(pathFunc, options = {}) {
        ...
        const checkPosition = () => {
            try {
                ...
                this.#logDebug(`Checking if bot is stuck during handlePath at position: ${this.vecToString(currentPosition)}, previousPosition: ${this.vecToString(previousPosition)}, isMining: ${this.#bot.pathfinder.isMining()}, chestIdOpen: ${this.#chestIdOpen}, isCrafting: ${this.isCrafting}`)
                if (currentPosition.distanceSquared(previousPosition) < 4 && !this.#bot.pathfinder.isMining() && !this.#chestIdOpen && !this.isCrafting) {
                    // if the bot hasn't moved > 2 blocks or performed other actions then we are stuck
        ...
        const timer = this.setInterval(checkPosition, interval);
        try {
            await pathFunc();
        } catch (err) {
            if (stuck) {
     ...
     * @param {Vec3} [options.originPoint=undefined] Origin point to search from, default is the bot current position
    ...
    findEntities(options = {} ) {
        ...
        const originPoint = options.originPoint || this.position()
        ...
        let result = Object.values(this.#bot.entities).filter(entity => {
            ...
            if(entity.isValid && (!entityNames || entityNames.length === 0 || entityNames.find(targetName => (entity.username && entity.username === targetName) || targetName === entity.name || (partialMatch && ( (entity.username && entity.username.includes(targetName)) || entity.name.includes(targetName)))))) {
                if(!attackable || (attackable && (entity.type === 'mob' || entity.type === 'player'))) {
                    const distance = originPoint.distanceTo(entity.position)
     ...
     * @param {Vec3} position The position to navigate to
     * @param {object} [options={}] Optional parameters
     * @param {number} [options.reach=1] How close to get to the position
     * @return {Promise<boolean>}
     */
    async approachPosition(position, options = {}) {
        const reach = options.reach || 1;
        try {
            const rangeToPosition = this.position().distanceTo(position)
            this.#logDebug(`Approaching ${position} from range: ${rangeToPosition}`);
            return await this.handlePath(async () => {
                await this.#bot.pathfinder.goto(new GoalNear(position.x, position.y, position.z, reach))
            })
        } catch(err) {
            this.#logWarn('Error approaching position', err);
        }
        return false
    }
    ...
    async equipBestHarvestTool(block) {
        ...
        if (bestHarvestTool) {
            try {
                ...
                return new Promise( resolve => resolve(this.#bot.heldItem));
            } catch (err) {
    ...
    findItemsOnGround(options = {}) {
        ...
        let result = Object.values(this.#bot.entities).filter((entity) => {
            ...
            const distance = distances[pt.id] || originPoint.distanceTo(pt.position)
    ...
    async findAndCollectItemsOnGround(options = {}) {
        ...
        const collectFunction = async (collector, collected) => {
            if (collected && collector.username === this.username()) {
                const anItem = this.getItemDefinitionById(collected.metadata[8]?.itemId)
                this.#logDebug(`I collected: ${collected.username || collected.metadata?.length > 8 && (anItem && anItem.name) || collected.name}`);
        ...
        if (quantityAvailable > 0) {
            ...
            try {
                ...
                while (quantityToDrop > 0 && i < itemsToDrop.length) {
                    ...
                    await this.#bot.toss(itemToDrop.type, itemToDrop.metadata, currentQuantity);
    ...
    async openContainer(containerBlock) {
        ...
        if (result != null) {
            ...
            this.#chestLeftOpenTimer = this.setInterval(checkChestLeftOpen, 5000)
        }
        return result
    }
    ...
    async closeContainer(containerWindow) {
        ...
        if (!containerWindow) {
            ...
            try {
                ...
                if (this.#chestLeftOpenTimer) {
                    this.clearInterval(this.#chestLeftOpenTimer)
                    this.#chestLeftOpenTimer = null
                }
