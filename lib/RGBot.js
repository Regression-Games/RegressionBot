 ...
 *             }
     ...
     * @param {exception} exception Optional - An exception to log with the message
     * @returns {void}
     */
     ...
     * @param {string} message The message to log
     * @param {exception} ex Optional - An exception to log with the message
     * @returns {void}
     */
    ...
    getEntityName(entity) {
        return (entity.username && entity.username) || (entity.objectType === "Item" && entity.onGround && entity?.metadata?.length > 8 && this.getItemDefinitionById(entity.metadata[8]?.itemId)?.name) || entity.name || null;
    }
    ...
    async handlePath(pathFunc, options = {}) {
        ...
        try {
            ...
            if (stuck) {
                ...
                this.#logWarn(`Caught GoalChanged during handlePath: ${err.message}.  This normally means that the target block/location/entity/etc for pathfinding no longer exists, that the pathing solution to the destination has become invalid, or that computing a path is taking too long.  Normally your bot should select a new action to take and move on.`)
            }
            return false
        } finally {
            clearInterval(timer);
     ...
     * @param {boolean} [options.skipClosest=false] Deprecated since 1.2.0 : No Longer used - Will attempt to locate the next-closest Block. This can be used to skip the closest Block when the Bot encounters an issue collecting it
     * @returns {Block | null}
     */
    findBlock(blockType, options = {}) {
        options['blockNames'] = blockType && [blockType] || [];
        return this.findBlocks(options).shift()?.result || null
    }
    ...
    findItemsOnGround(options = {}) {
        ...
        let result = Object.values(this.#bot.entities).filter((entity) => {
            ...
            const sortValue = sortValueFunction(distances[pt.id], value)
            return new FindResult(pt, sortValue)
        }).sort((a, b) => {
            return a.value - b.value
        });
    ...
    async collectItemOnGround(item) {
        ...
        if (item && item.objectType === "Item" && item.onGround) {
            ...
            try {
                ...
                //this.#logDebug(`Failed to approach item, error: ${ex}`)
            } finally {
